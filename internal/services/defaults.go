...
// DefaultHomepageJSON is the default homepage DIY page data for new tenants.
// Modeled after the reference site's homepage layout: banner, special (headlines),
// navBar (4 icons), and goods section.
const DefaultHomepageJSON = `{
  ...
  "items": [
    ...
    },
    {
      "name": "公告组",
      "type": "notice",
      "params": {"text": "春節放假通知", "icon": "/static/img/diy/notice.png"},
...
// CreateDefaultPageDesigns creates the default page designs for a new tenant.
// This is called both during initial seed (migration) and when a new user registers.
func CreateDefaultPageDesigns(db *gorm.DB, tenantID uuid.UUID) error {
	// Check if this tenant already has a default homepage
	var count int64
	db.Model(&models.PageDesign{}).Where(
		"tenant_id = ? AND type = ? AND is_default = ? AND trashed_at IS NULL",
		tenantID, "home", true,
	).Count(&count)

	if count > 0 {
		log.Printf("[defaults] Tenant %s already has a default homepage, skipping", tenantID)
		return nil
	}

	// Parse the default page data JSON
	var pageData models.JSONB
	if err := json.Unmarshal([]byte(DefaultHomepageJSON), &pageData); err != nil {
		log.Printf("[defaults] Error parsing default homepage JSON: %v", err)
		return err
	}

	homepage := models.PageDesign{
		TenantID:  &tenantID,
		Name:      "商城首页",
		Type:      "home",
		PageData:  pageData,
	...
	if err := db.Create(&homepage).Error; err != nil {
		log.Printf("[defaults] Error creating default homepage for tenant %s: %v", tenantID, err)
		return err
	}

	log.Printf("[defaults] Created default homepage (id=%s) for tenant %s", homepage.ID, tenantID)
	return nil
}
